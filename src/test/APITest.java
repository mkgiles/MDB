...
class APITest {
	...
	final void testGetActorRole() throws Exception {
		...
		assertEquals("Cast Away", API.getActorRole(API.getActor("Tom Hanks")).get(0).car().getTitle());
	}

	@Test
	final void testGetMovieRole() {
		assertEquals("Chuck Noland",
				API.getMovieRole(API.getMovie("Cast Away")).get(p -> p.cdr().equals("Chuck Noland")).cdr());
		assertEquals("Tom Hanks",
				API.getMovieRole(API.getMovie("Cast Away")).get(p -> p.cdr().equals("Chuck Noland")).car().getName());
	}
